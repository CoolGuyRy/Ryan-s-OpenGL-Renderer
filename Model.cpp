...
void Model::Load(std::string src) {
	...
	const aiScene* scene = importer.ReadFile(src, aiProcess_Triangulate | aiProcess_FlipUVs | aiProcess_JoinIdenticalVertices | aiProcess_PreTransformVertices | aiProcess_FindDegenerates);
...
void Model::Draw() {
	for (unsigned i = 0; i < mMeshes.size(); i++) {
		...
		GLuint dLightDirLoc = glGetUniformLocation(mShader->GetProgram(), "dLight[0].direction");
		GLuint dLightAmbientLoc = glGetUniformLocation(mShader->GetProgram(), "dLight[0].ambient");
		GLuint dLightDiffuseLoc = glGetUniformLocation(mShader->GetProgram(), "dLight[0].diffuse");
		GLuint dLightSpecularLoc = glGetUniformLocation(mShader->GetProgram(), "dLight[0].specular");
		GLuint dLightCountLoc = glGetUniformLocation(mShader->GetProgram(), "numDLights");
